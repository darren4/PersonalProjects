...
class PerpendicularSearch(BaseVectorSearch):
    def __init__(
        ...
        self._index = np.empty(
            (self._vector_len, self._search_range), dtype=object
        )
    ...
    def search(
        ...
        for dim in range(self._vector_len):
            vector_copy[dim] = self._normalize_vector_value(
                vector_copy[dim]
            )

        dim_coverage_cutoff = int(
            COVERED_DIM_CUTOFF_PROP * self._vector_len
        )
        search = {}
        results = []
        for abs_delta in range(SEARCH_DISTANCE):
            ...
            for delta in delta_options:
                for dim in range(self._vector_len):
                    next_value = min(
                        vector_copy[dim] + delta, self._max_index
                    )
