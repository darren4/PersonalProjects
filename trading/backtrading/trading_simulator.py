from trading.backtrading.strategy_interface import TradingWindow, TradeAction, Trade, Holdings, StrategyInterface
...
class TradingSimulator:
    def __init__(self, historical_data: List[TradingWindow], starting_cash: float) -> None:
    ...
    def backtrade_strategy(self, strategy: StrategyInterface) -> float:
        ...
        for trading_window in self._historical_data:
            account_assets_value: float = holdings.cash + (holdings.long_shares * trading_window.prices.open)
            account_liability_value: float = trading_window.prices.open * holdings.short_shares
            ...
            for trade in trades:
                if trade.action == TradeAction.OPEN_LONG:
                    ...
                    if holdings.cash < transaction_value:
                        raise ValueError(f"{holdings.cash} insufficient to buy {trade.amount} at closing price {transaction_share_price}")
                    holdings.cash -= transaction_value
                    holdings.long_shares += trade.amount
                elif trade.action == TradeAction.CLOSE_LONG:
                    transaction_share_count: int = min(trade.amount, holdings.long_shares)
                    transaction_value: float = transaction_share_count * transaction_share_price
                    holdings.cash += transaction_value
                    ...
                    exchange_difference: float = open_shorts[-1].price - transaction_share_price
